...
- **Board scene pieces:** reusable inline SVGs in `src/components/scene/` — `BranchSvg` (high perch limb + leaves), `ReedSvg` (reed clump on sand mound), `BankFoliageSvg` (corner meadow/sand shelves + pebbles/reeds), `MossTuftSvg` (mid-bank moss clumps), `RiverChannelSvg` (soft meandering watercolor channel under the puddles — multiply blend + damp-sand halo, not a rectangular blue spine), `WaterPuddleSvg` (watercolor puddles with depth wash, glare streak, and ripple arcs). Depth is faked with CSS: paper-grain overlay, bank washes, mid-bank moss, and slight downstream row scale/shadow — not CSS 3D perspective on interactive tiles.
