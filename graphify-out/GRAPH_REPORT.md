...
## Corpus Check
- 63 files · ~59,085 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 429 nodes · 976 edges · 17 communities (16 shown, 1 thin omitted)
...
1. `GameState` - 31 edges
2. `CardType` - 20 edges
3. `compilerOptions` - 17 edges
4. `scoreMove()` - 16 edges
5. `Board()` - 15 edges
6. `compilerOptions` - 15 edges
7. `reachableZones()` - 14 edges
8. `kingfisher()` - 14 edges
...
- `PassPlayGame()` --calls--> `speciesShort()`  [EXTRACTED]
  src/App.tsx → src/lib/presentation.ts
- `BotsGame()` --indirect_call--> `Board()`  [INFERRED]
  src/App.tsx → src/Board.tsx
- `BotsGame()` --calls--> `setup()`  [EXTRACTED]
  src/App.tsx → src/game/Game.ts
...
Cohesion: 0.05
Nodes (37): 10. Implementation Notes (CSS Modules), 11. Screens Inventory, 1. Art Direction — "Sunlit Field Guide", 2. Color Palette, 3. Typography, 4. Layout Structure, 5. Card & Component System, 6. The Theatrical Core: Select → Lock → Reveal → Resolve (+29 more)

### Community 1 - "Board.tsx"
Cohesion: 0.07
Nodes (44): App(), BotsGame(), BotsMode, Mode, PassPlayClient, PassPlayGame(), Board(), PendingSelection (+36 more)
...
### Community 4 - "Game.ts"
Cohesion: 0.11
Nodes (33): beforeHover, ev(), mm(), order, phaseLog, reducer, state, enumerateLegalMoves() (+25 more)
...
Cohesion: 0.14
Nodes (28): ACTION_DECK, VALID_CARDS, bestPlayer(), drawOne(), driftFish(), endOfRoundCleanup(), handReset(), nextPlayer() (+20 more)
...
### Community 18 - "bot.ts"
Cohesion: 0.09
Nodes (34): avg(), BotKind, main(), makeBot(), newStats(), report(), simulate(), Stats (+26 more)

## Knowledge Gaps
- **127 isolated node(s):** `Stats`, `BotKind`, `Stats`, `state`, `order` (+122 more)
...
- **Why does `GameState` connect `bot.ts` to `Board.tsx`, `Game.ts`, `types.ts`, `resolution.ts`, `cleanup.ts`, `legacy.ts`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `CardType` connect `Board.tsx` to `Game.ts`, `types.ts`, `resolution.ts`, `cleanup.ts`, `legacy.ts`?**
